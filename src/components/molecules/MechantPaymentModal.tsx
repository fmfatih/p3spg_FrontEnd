...
export function MechantPaymentModal({ transaction,isOpen, handleClose }) {
  ...
  const { mutate: GetMerchantPaymentDetail, isLoading: isGetMerchantPaymentDetailLoading } =
    useGetMerchantPaymentDetail();
  ...
  const columns: GridColDef[] = useMemo(() => {
    return [
      ...
      { field: "bankCommissionAmount", headerName: "Banka Komisyon Tutarı", width: 200 },
      { field: "bankName", headerName: "Banka Adı", width: 180 },
      { field: "bankblocked", headerName: "Banka Engellendi mi?", width: 180 },
      { field: "bankblockedday", headerName: "Banka Engelleme Günü", width: 200 },
      ...
      { field: "customerCommissionAmount", headerName: "Müşteri Komisyon Tutarı", width: 200 },
      ...
      { field: "merchantCommisionAmount", headerName: "İşyeri Komisyon Tutarı", width: 200 },
      { field: "merchantName", headerName: "İşyeri Adı", width: 180 },
      { field: "merchantblocked", headerName: "İşyeri Engellendi mi?", width: 180 },
      { field: "merchantblockedday", headerName: "İşyeri Engelleme Günü", width: 200 },
  ...
  useEffect(() => {
    if(isOpen && transaction)   {
      GetMerchantPaymentDetail({merchantId: transaction.merchantId, date: transaction.endOfDayDate,},  {
        onSuccess: (data) => {
          if (data.isSuccess) {
            setTableData(data.data);
            setSnackbar({
      ...
      },
      {
        onSuccess: (data) => {
          if (data.isSuccess) {
            ...
            downloadExcel(data?.data?.result || [], "İşyeri Hak Ediş Raporları Detay");
          } else {
            setSnackbar({
              severity: "error",
              description: data.message,
...
>
          ...
          <Stack justifyContent="center" sx={{ width: 1, height: 600, flexGrow: 1 }}>
